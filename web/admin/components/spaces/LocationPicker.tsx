...
export default function LocationPicker({ useFormPosition = false }: LocationPickerProps) {
  ...
  const form = watch(['id', 'lng', 'lat', 'address', 'name', 'timezone', 'parkingspaceMapview'])
  ...
  const setShapeCoordinates = (shapeRef: any, type: string) => {
    ...
    if (shapeRef.getPath().getLength()) {
      ...
      setValue('parkingspaceMapview', ployShapes)
    }
  }
  ...
  useEffect(() => {
    if (form.parkingspaceMapview && form.parkingspaceMapview.geojson.coordinates.length > 0) {
      setPolygonCoordinatesObject(frameCoordinates(form.parkingspaceMapview.geojson.coordinates[0]))
    }
  }, [])
