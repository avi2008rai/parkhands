...
export type SepaDebitFormProps = {
  onCreate?: (paymentMethodId: string) => void
  onFormSuccess?: () => void
}

export default function SepaDebitForm({ onFormSuccess, onCreate }: SepaDebitFormProps) {
    ...
    <FormContext {...methods}>
      <form onSubmit={onSubmitHandler} method="POST" encType="multipart/form-data">
        <Grid container direction="column" justify="space-evenly">
          <Grid item xs={12}>
            <NameController autoFocus disabled={loading} inputRef={cardHolder} label={t('Name')} />
          </Grid>
          <Grid item xs={12}>
            <IbanElement
              id="iban"
              className={classes.ibanElement}
              options={{
