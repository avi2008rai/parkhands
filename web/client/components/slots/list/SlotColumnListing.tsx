...
export default function SlotColumnListing() {
  ...
  const { space, slots, loading } = useSelectedSpace()
  ...
  return (
    <Box mx={2}>
      ...
      <Box mb={1}>
        ...
        />
        <Typography display="inline">{space?.name}</Typography>
      </Box>
            ...
            return (
              <Grid item key={slot.id}>
                <SlotItem
                  {...slot}
                  buttonProps={{
                    onClick: () => selectSlot({ slotId: slot.id, skipAnimation: true }),
                  }}
                />
              </Grid>
            )
