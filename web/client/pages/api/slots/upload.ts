...
const csvRecordToSlot = async (row: CsvRecord): Promise<Slot> => {
...
const handler = async (req: NextApiRequest, res: NextApiResponse, { client }: AuthContext) => {
  ...
  try {
    ...
    const { files }: FormFiles = await new Promise((resolve, reject) => {
    ...
    const imported = await Promise.all(
      _.map(data, async (row) => {
        try {
          const slot = await csvRecordToSlot(row)
