...
BACKUPDIR="/storage/postgresql/postgresql-backups"
...
# VER 1.1 - (2017-12-26)
#   Add config file parameter - based on AutoMySQLBackup 3.0
#
#=====================================================================
#=====================================================================
#
...
if (( $opt_flag_config_file )); then if [[ -r "${opt_config_file}" ]]; then source "${opt_config_file}"; else echo "Error: ${opt_config_file} can not be readen"; exit 1; fi; fi
...
if [ "$PEER_AUTH" = "yes" ]; then
  pg_dump -U $USERNAME -Fc $OPT -f $2 $1
else
...
compression () {
DIR_FILES=$(dirname "$1")
ROLES_FILE="$DIR_FILES/roles.sql"
dumproles $ROLES_FILE

cd $DIR_FILES && tar -cvf $1.tar $(basename $1) $(basename $ROLES_FILE) && rm $1 $ROLES_FILE && cd -
...
dumproles () {
if [ "$PEER_AUTH" = "yes" ]; then
       pg_dumpall -U $USERNAME $HOST --roles-only > $1
else
       pg_dumpall --username=$USERNAME -roles-only > $1
fi

python3 $FILTER_ROLES -f $1
return 0
}
...
if [ "$DBNAMES" = "all" ]; then
	if [ "$PEER_AUTH" = "yes" ]; then
          DBNAMES="`docker exec -i ph_db psql -U $USERNAME $HOST -l -A -F: | sed -ne "/:/ { /Name:Owner/d; /template0/d; s/:.*$//; p }"`"
        else
