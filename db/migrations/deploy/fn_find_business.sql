...
DROP FUNCTION api.find_business(payload api.find_business_input);
DROP TYPE api.find_business_input;
DROP TYPE api.find_business_result;
...
BEGIN
  ...
  FROM (
    ...
    AND (
      ...
      OR (
        _filter_owner_id
        AND business.owner_id = _owner_id
      )
    )
