  ...
  IF _distance_m > 10000 THEN
    RETURN QUERY
      SELECT
        ...
        , FALSE as booked
        , TRUE as in_working_hours
        , TRUE as in_amenities
        , FALSE as in_vs
  ...
  ELSE
    RETURN QUERY
        ...
        FROM timescale.slot_booking ts_sb
          ...
          AND (
            (ts_sb.start_time, ts_sb.end_time)
              OVERLAPS
            (_request_start_time, _request_end_time)
          )
      ...
      availabilities as (
        ...
        LEFT JOIN api.slot AS api_s ON api_s.id = api_sa.slot_id
          WHERE
            EXTRACT(DOW FROM _request_start_time::TIMESTAMPTZ AT TIME ZONE api_s.timezone) = api_sa.day_of_week
            AND tstzrange(
            (date(_request_start_time::TIMESTAMPTZ AT TIME ZONE api_s.timezone) + api_sa.start_hour::TIME)::TIMESTAMP AT TIME ZONE api_s.timezone,
            (date(_request_end_time::TIMESTAMPTZ AT TIME ZONE api_s.timezone) + api_sa.end_hour::TIME)::TIMESTAMP AT TIME ZONE api_s.timezone
          ) @> tstzrange(_request_start_time, _request_end_time)
