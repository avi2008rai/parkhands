    ...
    RETURN QUERY
      ...
      availabilities as (
        ...
        LEFT JOIN api.slot AS api_s ON api_s.id = api_sa.slot_id
          WHERE
            EXTRACT(DOW FROM _request_start_time) = api_sa.day_of_week
            AND tstzrange(
            (date(_request_start_time) + api_sa.start_hour::TIME)::TIMESTAMPTZ,
            (date(_request_end_time) + api_sa.end_hour::TIME)::TIMESTAMPTZ
