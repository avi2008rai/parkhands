...
BEGIN
  ...
  FROM api.slot_availability api_sa
    ...
    AND EXTRACT(DOW FROM _start_time) = api_sa.day_of_week
    AND (date(_start_time) + api_sa.start_hour::TIME)::TIMESTAMPTZ <= _start_time
    AND (date(_end_time) + api_sa.end_hour::TIME)::TIMESTAMPTZ >= _end_time;
