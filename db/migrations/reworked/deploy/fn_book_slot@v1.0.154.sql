...
DROP FUNCTION api.book_slot(payload api.book_slot_input);
DROP TYPE api.book_slot_input;
...
DECLARE
  ...
  _phone            phone_us DEFAULT payload.phone;
  _slot             api.slot;
BEGIN
  ...
  INSERT INTO timescale.slot_booking
    (slot_id, user_id, license_plate, start_time, end_time, phone)
  VALUES
    (
      _slot_id
      , _user_id
      , _license_plate
      , _start_time::TIMESTAMP AT TIME ZONE _slot.timezone
      , _end_time::TIMESTAMP AT TIME ZONE _slot.timezone
      , _phone
    )
