...
DECLARE
  ...
  _only_available        BOOLEAN DEFAULT COALESCE(payload.only_available, FALSE);
...
BEGIN
  ...
  ELSE 
  -- If not fitlering by only available slots
  -- get the available slots so the `available` field
  -- in the result set can be set accordingly.
    SELECT INTO slots_in_working_hours util.check_slot_availability((
        slots_in_distance
        , _request_start_time
        , _request_end_time
      )::util.check_slot_availability_input);
  ...
  RETURN QUERY
    SELECT * FROM (
      SELECT
        ...
        , (
          CASE
            WHEN _only_available THEN TRUE
            WHEN (cardinality(slots_in_working_hours ) > 0 AND slot.id = ANY (slots_in_working_hours )) THEN TRUE
            ELSE FALSE
          END
        ) AS in_working_hours
        FROM api.slot AS slot
