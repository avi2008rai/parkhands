...
BEGIN
  ...
  FROM api.slot_availability api_sa
    ...
    AND (date(now()::timestamp) + api_sa.start_hour::TIME <= _start_time AT TIME ZONE _slot.timezone)
    AND (date(now()::timestamp) + api_sa.end_hour::TIME >= _end_time AT TIME ZONE _slot.timezone);
