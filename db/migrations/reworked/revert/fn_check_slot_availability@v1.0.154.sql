...
BEGIN
  ...
  , slot_timetable AS (
    SELECT
      ...
      FROM (
        SELECT
          generate_series((sh.booking_date + sh.start_hour)::TIMESTAMP AT TIME ZONE sh.slot_timezone, (sh.booking_date + sh.end_hour - _timeslot_interval)::TIMESTAMP AT TIME ZONE sh.slot_timezone, _timeslot_interval) AS start_time
          , sh.booking_date
          , sh.day_of_week
          , sh.slot_id
