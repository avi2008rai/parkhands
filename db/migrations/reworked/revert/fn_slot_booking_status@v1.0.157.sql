...
BEGIN
  ...
  FROM api.slot_availability api_sa
    ...
    AND EXTRACT(DOW FROM _start_time::TIMESTAMPTZ) = api_sa.day_of_week
    AND ((date(_start_time::timestamp) + api_sa.start_hour::TIME)::TIMESTAMPTZ <= _start_time)
    AND ((date(_end_time::timestamp) + api_sa.end_hour::TIME)::TIMESTAMPTZ >= _end_time);
  ...
  FROM timescale.slot_booking ts_sb
    ...
    AND (
      (ts_sb.start_time, ts_sb.end_time)
        OVERLAPS
      (_start_time::TIMESTAMP AT TIME ZONE _slot.timezone, _end_time::TIMESTAMP AT TIME ZONE _slot.timezone)
    );
